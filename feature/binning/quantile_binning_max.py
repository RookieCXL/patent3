...
from federatedml.statistic.statics_cz import MultivariateStatisticalSummary_cz
...
class QuantileBinning(BaseBinning):
    ...
    def fit_split_points(self, data_instances):
        ...
        percent_value = 1.0 / (self.bin_num-1)

        # calculate the split points
        percentile_rate = [i * percent_value for i in range(0, self.bin_num)]
        LOGGER.info("cz  percentile_rate {} ".format(percentile_rate))
        ...
        LOGGER.info("cz  self.bin_results.all_split_points {} ".format(self.bin_results.all_split_points))
    ...
    def _fit_split_point(self, data_instances, is_sparse, percentile_rate):
        ...
        #chenzhong72 获取最小值
        statistics = MultivariateStatisticalSummary_cz(data_instances,
                                                    self.bin_inner_param.bin_indexes,
                                                    abnormal_list=self.abnormal_list)

        max_dict = statistics.get_max()
        for col_name, summary in summary_dict.items():
            # min_value = min_dict.get(col_name) - 0.1
            max_value = max_dict.get(col_name) + 0.1
            split_point = []
            for percen_rate in percentile_rate:
                s_p = summary.query(percen_rate)
                LOGGER.info("cz s_p{} ".format(s_p))
                if not self.allow_duplicate:
                    if s_p not in split_point:
                        split_point.append(s_p)
                        LOGGER.info("cz split_point{} ".format(split_point))
                else:
                    split_point.append(s_p)
                    LOGGER.info("cz split_point{} ".format(split_point))
