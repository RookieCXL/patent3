...
from federatedml.tree.hetero.my_hetero_decision_tree_host import MyHeteroDecisionTreeHost
...
class HeteroSecureBoostingTreeHost(BoostingTree):
    def __init__(self):
        ...
        self.result_weight_dict_record=None
    ...
    def sync_encrypted_predict_F_init(self):
        encrypted_predict_F_init = self.transfer_variable.encrypted_predict_F_init.get(idx=0)
        LOGGER.info("sync encrypted_predict_F_init {}".format(encrypted_predict_F_init))
        return encrypted_predict_F_init

    def sync_encrypted_predit_F_final(self, encrypted_predit_F_final):
        self.transfer_variable.encrypted_predit_F_final.remote(encrypted_predit_F_final,
                                                               role=consts.GUEST,
                                                               idx=0)

    @staticmethod
    def my_accumulate_f(f_val, new_f_val, lr=0.1, idx=0):
        # Copy from hetero_secureboosting_tree_guest
        f_val[idx] += lr * new_f_val
        return f_val

    def my_update_f_value(self, new_f=None, tidx=-1, mode="train"):
        # Copy from hetero_secureboosting_tree_guest
        LOGGER.info("update tree f value, tree idx is {}".format(tidx))
        accumulate_f = functools.partial(self.my_accumulate_f,
                                         lr=self.learning_rate,
                                         idx=tidx)
        self.encrypted_predict_F = self.encrypted_predict_F.join(new_f, accumulate_f)

    def my_predict_f_value(self, data_inst, predict_start_round, rounds):
        encrypted_predict_F_init = self.sync_encrypted_predict_F_init()
        self.encrypted_predict_F = encrypted_predict_F_init

        for i in range(predict_start_round, rounds):
            for tidx in range(self.tree_dim):
                tree_inst = MyHeteroDecisionTreeHost(self.tree_param)
                tree_inst.load_model(self.tree_meta, self.trees_[i * self.tree_dim + tidx])
                tree_inst.set_flowid(self.generate_flowid(i, tidx))
                tree_inst.set_runtime_idx(self.component_properties.local_partyid)

                enc_predict_data = tree_inst.predict(data_inst)
                self.my_update_f_value(new_f=enc_predict_data, tidx=tidx, mode="predict")
        #TODO 给guest方传回加密结果
        self.sync_encrypted_predit_F_final(self.encrypted_predict_F)
    ...
    def predict(self, data_inst, predict_param=None):
        ...
        self.predict_f_value(data_inst, predict_start_round, rounds)
        # self.my_predict_f_value(data_inst, predict_start_round, rounds)
