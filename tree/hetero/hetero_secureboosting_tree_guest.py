...
from federatedml.tree.hetero.my_hetero_decision_tree_guest import MyHeteroDecisionTreeGuest
...
class HeteroSecureBoostingTreeGuest(BoostingTree):
    ...
    def sync_encrypted_predict_F_init(self, encrypted_predict_F_init):
        # used for my_predict
        LOGGER.info("sync F_init {}".format(encrypted_predict_F_init))
        self.transfer_variable.encrypted_predict_F_init.remote(encrypted_predict_F_init,
                                                                role=consts.HOST,
                                                                idx=-1)

    def sync_encrypted_predit_F_final(self):
        sync_encrypted_predit_F_final = self.transfer_variable.encrypted_predit_F_final.get(0)
        return sync_encrypted_predit_F_final

    def my_predict_f_value(self, data_inst, cache_dataset_key):
        ...
        if last_round == -1:
            ...
            encrypted_predict_F_init = self.encrypted_calculator.encrypt(self.predict_F)
            # LOGGER.debug('jyp self.predict_F is {}'.format(list(self.predict_F.collect())))
            # LOGGER.debug('jyp encrypted_predict_F_init is {}'.format(list(encrypted_predict_F_init.collect())))
        else:
            LOGGER.debug("hit cache, cached round is {}".format(last_round))
            if last_round >= rounds - 1:
                LOGGER.debug(
                    "predict data cached, rounds is {}, total cached round is {}".format(rounds, last_round))
        ...
        self.sync_encrypted_predict_F_init(encrypted_predict_F_init)

        for i in range(last_round + 1, rounds):
            for tidx in range(self.tree_dim):
                tree_inst = MyHeteroDecisionTreeGuest(self.tree_param)
        ...
        #TODO 解密host方传回的结果
        encrypted_predit_F_final = self.sync_encrypted_predit_F_final()

        LOGGER.debug("jyp secureboost start to decreypt final score!")
        # self.predict_F = encrypted_predit_F_final
        # LOGGER.info("cxl encrypted_predit_F_final:".format(encrypted_predit_F_final))
        self.predict_F = self.encrypted_calculator.decrypt(encrypted_predit_F_final)
        LOGGER.debug("jyp secureboost decreypt final score end!")
    ...
    def predict(self, data_inst):
        ...
        LOGGER.info("jyp secure boost predict end")
    ...
    def set_model_param(self, model_param):
        ...
        self.classes_ = list(map(round, map(float, model_param.classes_)))
